---
title: "Toluene Normalizing and Plotting for SynFire Data"
author: Sierra
output:
  html_document:
    theme: cerulean
...
csv_files<- list.files(path=project.input.dir, pattern = "Toluene_PreProcessed", recursive = T, full.names = T) 

#Make vector for what the names of the basenames of the data files
data_frame_names<-gsub("2-4.csv","",basename(csv_files))
...
All_Plates_Raw<-Toluene_PreProcessed_Plates
...
P_wmfr<-ggplot(All_Plates_Means,aes(x=interval,y=Mean_wmfr,color=Dose,group=Dose))+
  ...
  theme_minimal()+
  ggtitle("SynFire: Weighted Mean Firing Rate (Raw)")
P_wmfr
```
...
P_nae<-ggplot(All_Plates_Means,aes(x=interval,y=Mean_nae,color=Dose,group=Dose))+
  ...
  scale_color_manual(values=c("black","darkgreen","aquamarine3","turquoise3","dodgerblue4","purple4","orchid"))+
  theme_minimal()+
  ggtitle("SynFire: Number of Active Electrodes (Raw")
P_nae
```

#Isolate Basline recordings 
```{r}
...
#find baseline nae and wmfr by well, plate, identifier, and dose
...
#save
ggsave("Synfire_wmfr_LinePlot_Normalized_AEfiltered.png",plot=wmfr_Norm,bg="white",width=10,height=4.5)
```

#Line Graph for NAE
```{r}
#Find the mean for NAE
...
#save
ggsave("Synfire_nae_LinePlot_Normalized_AEfiltered.png",plot=nae_Norm,bg="white",width=10,height=4.5)
```
